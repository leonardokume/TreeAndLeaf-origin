...
#' Test fucntion to help usability.
#'
#' @param g An igraph object.
#' @return An igraph object.
#' @examples
#' removeInternalNodes(g)
removeInternalNodes <- function(g){
  edgelist <- get.edgelist(g)
  idx <- edgelist[,2] %in% edgelist[,1]
  V(g)$nodeSize[idx] <- 1
  return(g)
}
...
getNodeIndex <- function(g){
  edgelist <- get.edgelist(g)
  idx <- !(edgelist[,2] %in% edgelist[,1])
  relevant <- edgelist[idx,2]
  idx <- match(V(g)$name, relevant)
  return(idx)
}
